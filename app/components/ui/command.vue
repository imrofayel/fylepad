<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <div
          class="shadow-notion relative overflow-hidden rounded-2xl border border-[#c6c6c4] bg-white/90 backdrop-blur-lg dark:border-[#525252] dark:bg-[#404040]/90"
        >
          <div
          ...
          <div class="max-h-96 overflow-y-auto">
            ...
            <div
              ...
              class="p-4 pt-3 text-center text-lg text-[#32302c] dark:text-gray-50"
            >
              No results found
            </div>
            ...
            <div v-else-if="!searchQuery" class="p-2">
              <div class="space-y-1">
                ...
                >
                  <div
                    class="mr-3 flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 dark:bg-[#333333]"
                  >
                    <svg
            ...
            <div v-else class="py-2">
              <Motion
                ...
                :class="[
                  'm-2 flex cursor-pointer items-start px-4 py-3 transition-colors last:border-b-0',
                  selectedIndex === index && '',
                ]"
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  <div class="mb-1 truncate text-base font-medium text-[#32302c] dark:text-gray-50">
                    {{ result.tabTitle || "Untitled" }}
                  </div>
                  <div
                    class="text-[15px] leading-relaxed text-[#6f6e6b] dark:text-gray-300"
                    v-html="result.highlightedText"
                  />
                </div>

                <kbd
                  class="px-0.5 py-0.5 dark:text-gray-50"
                  :class="selectedIndex !== index && 'hidden'"
                >
                  <svg
                    aria-hidden="true"
                    role="graphics-symbol"
          ...
          <div
            class="border-[#c6c6c4] px-2.5 py-2 text-[16.5px] text-[#32302c] dark:border-[#525252] dark:text-gray-50"
          >
...
const extractTextFromContent = (content: Record<string, unknown> | string): string => {
  if (!content) return "";
...
const highlightText = (text: string, searchTerm: string): string => {
  ...
  return text.replace(
    regex,
    '<mark class="bg-yellow-200 dark:bg-yellow-600/50 px-1 py-0.5 rounded">$1</mark>',
  );
};
...
const _getContext = (sentences: string[], index: number): string => {
...
// Get tab color class
const _getTabColorClass = (colorName: string) => {
...
:deep(mark) {
  @apply rounded bg-yellow-200 px-1 py-0.5 dark:bg-yellow-600/50;
}
</style>
