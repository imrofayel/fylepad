<template>
  <div v-if="groups.length > 0" class="mb-2">
    <div v-for="group in groups" :key="group.name" class="mb-1">
      <div
        class="flex cursor-pointer items-center justify-between rounded bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 dark:bg-[#2a2a2a] dark:text-gray-400"
      ...
      <div v-show="expandedGroups.has(group.name)" class="mt-1 ml-2 space-y-1">
        <div
          v-for="tab in group.tabs"
          :key="tab.index"
          class="flex cursor-pointer items-center justify-between rounded px-2 py-1 text-sm hover:bg-gray-100 dark:hover:bg-[#404040]"
          :class="{
        ...
        >
          <div class="flex items-center space-x-2">
            <div class="h-3 w-3 rounded-full" :class="getTabColorClass(tab.color)" />
...
interface Emits {
  (e: "tabSelected" | "closeTab", index: number): void;
}
