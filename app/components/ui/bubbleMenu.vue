<template>
  <bubble-menu
    ...
    :tippy-options="{ duration: 100, offset: [10, -70] }"
  >
    <div
      class="drop-shadow-cool flex overflow-hidden rounded-xl border border-[#c6c6c4] bg-white text-black dark:border-[#525252] dark:bg-[#404040] dark:text-gray-50 dark:text-white/85"
    >
      <button
        :class="{ 'bg-gray-100 dark:bg-[#171717]': editor.isActive('bold') }"
        class="rounded-l-lg p-2 px-2 hover:bg-gray-100 hover:dark:bg-[#171717]"
      ...
      <button
        ...
        class="p-2 px-2 hover:bg-gray-100 hover:dark:bg-[#171717]"
      ...
      >
        <div
          class="h-5 w-5 rounded-full border border-black/50 bg-yellow-400 dark:border-none dark:bg-yellow-500"
        />
      </button>
      ...
      <button
        v-if="isSpeechSynthesisAvailable"
        :class="{
          'is-reading': isReading,
          'hover:bg-gray-100 hover:dark:bg-[#171717]': true,
          'border-l border-[#c6c6c4] p-2 px-2 dark:border-[#525252]': true,
        }"
      ...
      <button
        v-if="isSpeechSynthesisAvailable"
        :class="{
          'is-active': showVoiceSettings,
          'hover:bg-gray-100 hover:dark:bg-[#171717]': true,
          'border-gray-200 p-2 px-2 dark:border-[#525252]': true,
        }"
    ...
    <div class="p-4">
      <div class="flex items-center justify-between">
        <div class="flex w-full items-center justify-between gap-1">
          <button class="play-pause-btn" aria-label="Pause/Resume reading" @click="pauseReading">
            <svg
              v-if="isPaused"
              xmlns="http://www.w3.org/2000/svg"
              width="18"
          ...
          <div class="audio-visualizer">
            <div
              ...
              :style="{
                height: `${audioVisualizerHeight[i % audioVisualizerHeight.length] + 4}px`,
              }"
            />
          </div>
  ...
  >
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-medium opacity-95">Voice Settings</h3>
      <button
        class="close-btn relative -top-2"
        aria-label="Close voice settings"
        @click="(e) => toggleVoiceSettings(e)"
      >
        ×
      </button>
    </div>
    <div class="voice-controls">
      ...
      <div class="form-group">
        ...
        <div class="speed-controls mt-2 flex flex-wrap gap-2">
          <button
...
const freeModels = ref<{ slug: string; name?: string }[]>([]);
...
// Computed properties
const _readingProgressPercent = computed(() => {
  return Math.round(readingProgress.value);
});

const _readingDurationFormatted = computed(() => {
...
const loadVoices = () => {
  ...
  if (availableVoices.value.length > 0) {
    ...
    const matchingVoice = availableVoices.value.find((v) =>
      v.lang.includes(userLang.split("-")[0]),
    );
    ...
    utterance.onboundary = (event: SpeechSynthesisEvent) => {
      if (event.name === "word" && currentTextRange.value) {
        ...
        try {
          ...
          if (props.editor && !isMinimized.value) {
            // Only highlight when not minimized
...
const startVisualizerAnimation = () => {
  ...
  const updateVisualizer = () => {
    ...
    if (words.value.length > 0 && currentWord.value < words.value.length) {
      ...
      currentWordDisplay.value = snippet.length > 40 ? snippet.substring(0, 37) + "..." : snippet;
    }
...
const setVoiceRate = (rate: number) => {
  ...
  if (window.speechSynthesis.speaking) {
    ...
    const currentPos = window.speechSynthesis.speaking
      ? Math.floor(
          ((Date.now() - readingStartTime.value) / readingDuration.value) * words.value.length,
        )
      : currentWord.value;
...
.ai-bar-inner {
  ...
  padding: 6px 10px;
  gap: 8px;
  animation: fadeInScale 0.25s cubic-bezier(0.4, 2, 0.6, 1) both;
}
.ai-bar-input {
  flex: 1;
  border: none;
