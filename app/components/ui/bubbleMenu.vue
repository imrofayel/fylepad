<template>
      <bubble-menu v-if="editor" :editor="editor as any" :tippy-options="{ duration: 100, offset: [10, -70] }">
        <div
...
:class="{ 'bg-gray-100 dark:bg-[#171717]': editor.isActive('link') }"
          ...
          <path
d="M9.5 14.5L14.5 9.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
...
:class="{ 'bg-gray-100 dark:bg-[#171717]': editor.isActive('highlight') }"
          ...
          <button
v-if="isSpeechSynthesisAvailable" :class="{ 'is-reading': isReading, 'hover:dark:bg-[#171717] hover:bg-gray-100': true, 'p-2 px-2 border-l border-[#c6c6c4] dark:border-[#525252]': true }"
          ...
          <button
v-if="isSpeechSynthesisAvailable" :class="{ 'is-active': showVoiceSettings, 'hover:dark:bg-[#171717] hover:bg-gray-100': true, 'p-2 px-2 border-gray-200 dark:border-[#525252]': true }" 
      ...
      <transition name="fade">
        <div v-if="showAIBar" class=" rounded-2xl drop-shadow-sm w-[90%] sm:w-[60%] md:w-[40%] left-1/2 -translate-x-1/2 top-8  fixed rainbow-border-effect-2 bg-white dark:bg-[#232323] dark:text-white backdrop-blur-2xl z-[1000000000]">
          <div class="flex px-2">
            ...
            >
            <button :disabled="aiLoading || !aiPrompt.trim()" class="!bg-[#24d86c] dark:!bg-[#0c843c] dark:!border-[#196838] !border-[#28c76d] text-white relative top-1 rounded-full max-h-fit p-[5px]" @click="submitAIPrompt">
              <span v-if="!aiLoading">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v14m7-7l-7-7l-7 7"/></svg>
              </span>
              <span v-else class="">
          ...
          <div class="flex w-full items-center absolute top-12 justify-between">
            ...
            <span :disabled="aiLoading" class="cursor-pointer border p-[3px] rounded-xl dark:!bg-[#232323] bg-white drop-shadow-sm dark:border-[#525252]  text-[15px] border-gray-100" @click="closeAIBar">
      ...
      <div v-if="isSpeechSynthesisAvailable && isReading" class="mini-controls rainbow-border-effect-2 dark:bg-[#404040] bg-white/80 backdrop-blur-xl text-black !rounded-2xl shadow-lg overflow-hidden transition-all duration-300 ease-in-out">
          ...
          <div class="p-4">
            <div class="flex items-center justify-between">
              ...
              <div
v-for="i in 30" :key="i" class="visualizer-bar" 
                  :style="{ height: `${audioVisualizerHeight[i % audioVisualizerHeight.length] + 4}px` }"/>
            </div>
...
v-if="isSpeechSynthesisAvailable && showVoiceSettings" 
        ...
        <div class="flex items-center justify-between">
          ...
          <button class="close-btn relative -top-2" aria-label="Close voice settings" @click="(e) => toggleVoiceSettings(e)">
            ×
          </button>
        </div>
