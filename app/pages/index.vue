<template>
  <div class="flex h-full w-full gap-0" :class="isVerticalTabs ? 'flex-row' : 'flex-col'">
    <!-- Command Dialog -->
    <UiCommand
      :is-open="commandOpen"
      :tabs="tabs"
    ...
    >
      <div class="flex items-center justify-center space-x-2 overflow-visible">
        ...
        <div ref="tabContainer" class="dropdown-menu flex space-x-2 overflow-auto">
          <div class="flex space-x-2">
            ...
            >
              ...
              <div
                ...
                class="relative right-0.5 mr-[4px] h-[16px] w-[4.5px] rounded-full"
              ...
              <button
                v-if="!tab.lock"
                class="ml-2 hover:text-red-600 dark:text-gray-50/50 dark:hover:text-gray-100"
                title="Close tab"
    ...
    >
      <!-- Sidebar Header -->
      <div
        class="flex items-center justify-between p-2.5 py-2 pr-1.5 dark:border-[#525252] dark:bg-[#171717]"
      >
        <!-- Tab Layout Toggle -->
        <button
      ...
      <!-- Vertical Tabs List -->
      <div ref="tabContainer" class="flex-1 space-y-1 overflow-y-auto p-2">
        <div
          v-for="(tab, tabIndex) in tabs"
          :key="tab.id"
          :ref="(el) => setTabRef(tabIndex, el)"
    ...
    <TabContextMenu
      ...
      :tab-color="
        contextMenu.tabIndex >= 0 ? tabs[contextMenu.tabIndex]?.color || 'Default' : 'Default'
      "
    ...
    if (editor) {
      ...
      setTimeout(() => {
        ...
        if (results && results.length > 0) {
          ...
          if (result.from !== undefined) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            bestMatchIndex = results.findIndex((r: any) => Math.abs(r.from - result.from) < 10);
      ...
      if (proseMirrorElement) {
        ...
        const fallbackEditor =
          vueComponent?.parent?.setupState?.editor || vueComponent?.ctx?.editor;
        ...
        if (fallbackEditor) {
          ...
          setTimeout(() => {
            ...
            if (results && results.length > 0) {
              ...
              if (targetResult) {
                ...
                setTimeout(() => {
                  try {
                    const { node } = fallbackEditor.view.domAtPos(
                      fallbackEditor.state.selection.anchor,
                    );
                    if (node instanceof HTMLElement) {
                      node.scrollIntoView({ behavior: "smooth", block: "center" });
                    }
          ...
          let node;
          const walker = document.createTreeWalker(proseMirrorElement, NodeFilter.SHOW_TEXT, null);
...
watch(
  [tabs, activeTab, () => colorMode.preference, isVerticalTabs, sidebarVisible],
  async () => {
    await saveAppState();
  },
  { deep: true },
...
useHead({
  ...
  meta: [{ name: "description", content: "A minimal notepad with literally everything you need." }],
});
